...
def get_data(crypto, date=np.array(1)):
    ...
    if type(date) == str:
        klines = client.get_historical_klines(crypto+'USDT', Client.KLINE_INTERVAL_4HOUR, date)
    else:
...
class data_process():
    def __init__(self, df_klines, is_binance = True):
        #  creation du df a partir des données Binance

        if is_binance:
            df_klines = pd.DataFrame(np.array(df_klines)[:, 0:11],
                                     index=np.arange(len(df_klines)),
        ...
        df_klines = df_klines.set_index('index')
